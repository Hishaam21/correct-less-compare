...
import { aggregateResults, filterAndRankResults } from './backend/scraperManager.js'
...
let searchProducts = async (query) => {
  try {
    const rawResults = await aggregateResults(query)
    return filterAndRankResults(rawResults, query)
  } catch (err) {
...
// Split multi-product queries by commas
async function searchMultipleProducts(query) {
  const items = query.split(',').map(item => item.trim().toLowerCase()).filter(item => item.length > 0)
  
  if (items.length < 2) {
    return await searchProducts(query)
  }

  console.log(`🔀 [SEARCH] Searching for ${items.length} items: ${items.join(', ')}`)
  ...
  for (const item of items) {
    try {
      ...
      console.log(`⚠️  Error searching for "${item}": ${err.message}`)
    }
  }

  const combined = Object.values(allResults)
  console.log(`✓ [SEARCH] Combined ${items.length} items into ${combined.length} results`)
  return combined
}
...
app.post('/api/search', async (req, res) => {
  try {
    ...
    const normalizedQuery = cleanQuery
      .replace(/\b(?:cheap|cheapest|best price|best|find|deal|for|price)\b/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim() || cleanQuery

    console.log(`\n🔍 [SEARCH] Query: "${cleanQuery}" -> "${normalizedQuery}" ${fresh ? '(fresh)' : '(cached)'}`);
    ...
    if (!results || results.length === 0) {
      try {
        console.log(`🌐 [SEARCH] Scraping websites for "${normalizedQuery}"...`)
        results = await searchMultipleProducts(normalizedQuery)

        // Step 3: Cache the filtered results
        if (results && results.length > 0) {
      ...
      if (!results || results.length === 0) {
        console.log(`🔍 Live search for "${cleanQuery}" returned no live scraper results.`)
        results = []
      }
    }
    ...
    res.json({
      ...
      message: fromCache
        ...
        : fallback
          ? `Live scrape returned no results; showing local fallback products.`
