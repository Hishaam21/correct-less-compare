...
async function handleSearchRequest(req, res) {
  try {
    const query = String(req.query.query || req.body?.query || '').trim().toLowerCase()
    const forceFresh = (req.query.fresh === 'true') || req.body?.fresh === true
    ...
    return res.status(500).json({
      error: 'Search failed',
      query: String(req.query.query || req.body?.query || '').trim().toLowerCase(),
      cached: false,
