/**
 * Woolworths South Africa Retailer Scraper
 * Scrapes Woolworths search results only
...
export async function scrapeWoolworths(query) {
  try {
    console.log(`🛒 [Woolworths] Searching for: "${query}"`)
    
    await randomDelay()

    const searchUrl = `${BASE_URL}/search?searchText=${encodeURIComponent(query)}`
    ...
    console.error(`❌ [Woolworths] Scraping failed:`, error.message)
    throw error
  }
}
